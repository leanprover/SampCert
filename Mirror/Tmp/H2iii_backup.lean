...
theorem T1 : ExceptT String (StateT BitStream Option) T = Nat := by
  unfold ExceptT
  unfold StateT
  sorry

theorem T2 : StateT BitStream Option T = Nat := by
  unfold StateT
  sorry

theorem T3 : Except String (StateT BitStream Option T) = Nat := by
  unfold StateT
  sorry

def Coin : ExceptT String (StateT BitStream Option) Bool := do
  let rng ← get
  let f: BitStream := λ n : Nat => rng (n + 1)
  set f
  return (rng 0)

def countable (f : ExceptT String (StateT BitStream Option) T) : Prop :=
  Countable { y : T | ∃ (g: StateT BitStream Option T) (x : BitStream) (r : BitStream ), f = g ∧  g.run x = (y,r) }

def prob_while_cut (cond : T → Bool) (body : T → ExceptT String (StateT BitStream Option) T) (n : Nat) (a : T) : ExceptT String (StateT BitStream Option) T :=
  match n with
  | zero => return a
...
noncomputable def prob_while (cond : T → Bool) (body : T → ExceptT String (StateT BitStream Option) T) (a : T) : ExceptT String (StateT BitStream Option) T := do
  let s ← get
...
noncomputable def prob_until (body : ExceptT String (StateT BitStream Option) T) (cond : T → Bool) : ExceptT String (StateT BitStream Option) T := do
  try
    let v ← body
...
def uniformP2 (k : Nat) : ExceptT String (StateT BitStream Option) Nat := do
...
-- Sample from [0..n)
noncomputable def uniform (n : Nat) : ExceptT String (StateT BitStream Option) Nat := do
  prob_until (uniformP2 (log 2 n + 1)) (λ x : Nat => x < n)

noncomputable def bernoulli (r : Rat) : ExceptT String (StateT BitStream Option) Bool := do
  if r < 0 || r > 1 then throw "Argument for Bernoulli not in range"
  let d ← uniform r.den
  return d < r.num

noncomputable def bernoulli_exp_neg_loop1 (gamma : Rat) (K : Bool × Nat) : ExceptT String (StateT BitStream Option) (Bool × Nat) := do
  let A ← bernoulli (gamma / (K.2))
  return (A, K.2 + 1)

noncomputable def bernoulli_exp_neg_le1 (gamma : Rat) : ExceptT String (StateT BitStream Option) Bool := do
  let r ← prob_while (λ K : Bool × Nat => K.1) (bernoulli_exp_neg_loop1 gamma) (true,1)
  return r.1

noncomputable def bernoulli_exp_neg_loop2 (iter : Nat) : ExceptT String (StateT BitStream Option) Bool := do
...
noncomputable def bernoulli_exp_neg (gamma : Rat) : ExceptT String (StateT BitStream Option) Bool := do
  if 0 ≤ gamma && gamma <= 1
...
noncomputable def laplace_loop1 (t : Nat) :  ExceptT String (StateT BitStream Option) (Nat × Bool) := do
  let U ← uniform t
  let D ← bernoulli_exp_neg (U / t)
  return (U,D)

noncomputable def laplace_loop2 (K : Bool × Nat) : ExceptT String (StateT BitStream Option) (Bool × Nat) := do
  let A ← bernoulli_exp_neg_le1 (-1)
  return (A, K.2 + 1)

noncomputable def laplace_body (t s : Nat) : ExceptT String (StateT BitStream Option) (Bool × Nat × Int) := do
...
noncomputable def laplace (t s : Nat) : ExceptT String (StateT BitStream Option) Int := do
  let r ← prob_until (laplace_body t s) (λ x : Bool × Nat × Int => ¬ x.1 || x.2.1 ≠ 0)
  return r.2.2

noncomputable def gaussian_loop (sigmasquared : Rat) (t : Nat) : ExceptT String (StateT BitStream Option) (Int × Bool) := do
  let Y ← laplace t 1
  let C ← bernoulli_exp_neg ((abs Y - sigmasquared / t)^2 / (2 * sigmasquared))
  return (Y,C)

noncomputable def gaussian (sigma : Rat) : ExceptT String (StateT BitStream Option) Int := do
