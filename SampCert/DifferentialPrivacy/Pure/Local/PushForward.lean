...
lemma push_forward_prob_is_prob_gen {T S : Type} [DecidableEq S] (p : SLang T) (f : T -> S) (h : ∑' (t : T), p t = n) :
  ∑' (s : S), (push_forward p f) s = n := by
    simp [push_forward]
    rw [@ENNReal.tsum_comm]
    have _: ∀b : T, ∑' (a : S), (if f b = a then p b else 0 : ENNReal) = p b := by
      intro b
...
lemma push_forward_prob_is_prob {T S : Type} [DecidableEq S] (p : SLang T) (f : T -> S) (h : ∑' (t : T), p t = 1) :
  ∑' (s : S), (push_forward p f) s = 1 := push_forward_prob_is_prob_gen p f h
