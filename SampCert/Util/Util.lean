...
theorem sum_simple (bound : ℕ) (k : ENNReal) :
 (∑' (a : ℕ), if a < bound then k else 0) = k * bound := by
  ...
  have B := @Summable.sum_add_tsum_nat_add' ENNReal _ _ _ _ (fun a => if a < bound then k else 0) bound A
  rw [← B]
  clear B
...
theorem tsum_split_ite (cond : T → Bool) (f g : T → ENNReal) :
  ...
  have B := @Summable.tsum_add_tsum_compl ENNReal T _ _ (fun i => if cond i then f i else g i) _ _ { i : T | cond i} ENNReal.summable ENNReal.summable
  rw [← B]
  clear B
...
theorem tsum_split_ite' (cond : T → Bool) (f g : T → ENNReal) :
  ...
  have B := @Summable.tsum_add_tsum_compl ENNReal T _ _ (fun i => if cond i = false then f i else g i) _ _ { i : T | cond i = false} ENNReal.summable ENNReal.summable
...
theorem tsum_shift_1 (f : ℕ → ENNReal) :
  ...
  rw [ENNReal.tsum_eq_iSup_nat]
  have A : Monotone (fun i => ∑ a ∈ Finset.range i, if a = 0 then 0 else f (a - 1)) := by
...
theorem tsum_shift'_1 (f : ℕ → ENNReal) :
  ...
  rw [ENNReal.tsum_eq_iSup_nat]
  rw [ENNReal.tsum_eq_iSup_nat]
  have A : Monotone fun i => ∑ a ∈ Finset.range i, if a = 0 then 0 else f a := by
    apply monotone_nat_of_le_succ
    intro n
    rw [sum_range_succ]
    simp
...
theorem tsum_shift'_2 (f : ℕ → ENNReal) :
  ...
  rw [ENNReal.tsum_eq_iSup_nat]
  rw [ENNReal.tsum_eq_iSup_nat]
  have A : Monotone fun i => ∑ a ∈ Finset.range i, if a = 0 then 0 else if a = 1 then 0 else f a := by
    apply monotone_nat_of_le_succ
    intro n
    rw [sum_range_succ]
    simp
...
lemma nat_div_eq_le_lt_iff {a b c : ℕ} (Hc : 0 < c) : a = b / c <-> (a * c ≤ b ∧ b < (a +  1) * c) := by
  ...
  · intro ⟨ H1, H2 ⟩
    ...
    · apply Nat.lt_succ_iff.mp
      exact (Nat.div_lt_iff_lt_mul Hc).mpr H2
